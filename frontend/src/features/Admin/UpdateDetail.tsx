      ...
      render={({ field }) => (
        <FormItem>
          ...
          <div className="grid grid-cols-1 gap-4 rounded-lg border p-4 md:grid-cols-2">
            <FormControl>
              <TiptapEditor value={field.value?.rawHtml || ""} onChange={(newContent) => field.onChange(newContent)} />
            </FormControl>
...
export default function UpdateDetailProduct() {
  ...
  const { fetchListBrand, data: brands } = useFetchBrand();
  const { FetchProductByID, dataID, loading } = useFetchProductByID(String(id));
  ...
  }, [id, fetchListBrand, FetchProductByID]);
  ...
  } = useFieldArray({ control, name: "main_images_detail" });
  const {
    fields: subImageFields,
    append: appendSubImage,
    remove: removeSubImage,
  } = useFieldArray({ control, name: "sub_images_detail" });
  ...
  async function onSubmit(values: ProductFormValues) {
    ...
    try {
      const response = await httpClient.put(`/admin/manage-products/update-product/${id}`, payload);
      if (response.status === 200) {
        toast.success("Thành công!", { description: "Sản phẩm đã được cập nhật thành công." });
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <Typography>Cập nhật các thông tin chi tiết cho sản phẩm của bạn.</Typography>
                    </CardHeader>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <Typography>
                                Đây là đường dẫn đầy đủ mà khách hàng sẽ truy cập để xem sản phẩm.
                              </Typography>
                              <FormMessage />
                            </FormItem>
                          )}
